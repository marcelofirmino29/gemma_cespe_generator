# generator/views/views_validators.py
...
from django.http import JsonResponse, HttpRequest # <<<<<<<<<<<< HttpRequest IMPORTADO AQUI
...
from ..forms import (
    ...
    # ... (outros formulários que este arquivo realmente usa)
) #
from ..models import (
    Avaliacao,
    Questao,
    TentativaResposta,
    # ... (outros modelos que este arquivo realmente usa)
) #
from ..utils import STOP_WORDS_PT #
from .. import scraper_logic #

# REMOVIDA a importação de 'call_gemma_model_ask_ai'
# from ..services import call_gemma_model_ask_ai # <<<<<<<<<<<< LINHA REMOVIDA

# A função _get_base_context_and_service (que você já tem em views_generate_questions.py)
# deve retornar uma instância do QuestionGenerationService (definido em services.py)
# ou algo que tenha um método compatível como get_ai_response.
from .views_generate_questions import _get_base_context_and_service #
from ..exceptions import AIResponseError, AIServiceError #

logger = logging.getLogger(__name__)

# --- landing_page_view (Mantida como você forneceu, com HttpRequest) ---
@login_required
def landing_page_view(request: HttpRequest): #
    ...
    try:
        ...
        for q in questoes_recentes: #
            ...
            if q.texto_comando: textos_combinados += q.texto_comando + " " #
        if not textos_combinados: #
            ...
            if not palavras_filtradas: logger.info("LandingPage: Nenhuma palavra relevante para nuvem.") #
            else:
    ...
    try:
        ...
        if soup: #
            ...
            elif err_extract: logger.info(f"LandingPage: Mensagem extração CNB (sem dados): {err_extract}") #
        else: logger.error("LandingPage: Falha obter soup CNB.") #
    except Exception as e:
...
# --- validate_answers_view (Mantida como você forneceu, com HttpRequest) ---
@login_required
def validate_answers_view(request: HttpRequest): #
    context, _, _ = _get_base_context_and_service(request) #
            ...
            for q_id in questao_ids_respondidas: #
                ...
                try:
                    ...
                    attempt_results.append({ #
                        'questao_id': q_obj.id, 'afirmacao': q_obj.texto_comando, #
...
# --- validate_single_ce_view (Mantida como você forneceu, com HttpRequest) ---
@login_required
@require_POST
def validate_single_ce_view(request: HttpRequest): #
    try:
        ...
        try: questao_id_int = int(questao_id) #
        except ValueError: #
    ...
    if request.method == 'GET': #
        ...
        if question_from_url: #
            ...
            if service_initialized and service: #
                try:
                    # O método get_ai_response em services.py espera 'user_prompt'
                    # O 'service' aqui é uma instância de QuestionGenerationService
                    ai_response_text = service.get_ai_response(user_prompt=question_from_url) # <<<<<<<<<<<< MUDANÇA AQUI
                    ...
                    logger.error(f"ASK_AI_VIEW (GET com param): Método 'get_ai_response' não encontrado no objeto service ({type(service)}). Verifique _get_base_context_and_service.") #
        ...
        if form.is_valid(): #
            ...
            if service_initialized and service: #
                try:
                    # O método get_ai_response em services.py espera 'user_prompt'
                    ai_response_text = service.get_ai_response(user_prompt=user_input_value) # <<<<<<<<<<<< MUDANÇA AQUI
                    ...
                    logger.error(f"ASK_AI_VIEW (POST): Método 'get_ai_response' não encontrado no objeto service ({type(service)}). Verifique _get_base_context_and_service.") #
